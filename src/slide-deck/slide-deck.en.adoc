...
to avoid some errors and bugs as early as possible...
and small bonus,

[slide=needs, display=12]
_

if we can collaborate with muggles, non-devs,
it's nice.
...
Here again, context is key.
...
So if you're a bit behind what happened in the frontend in the past few years, you may be thinking:
...
Then JavaScript arrived and we opened the door of complexity land.

image::img/drawings/browser-8.jpg[]

The JS engine was introduced,
and for the 1st time devs had the possibility to react directly to user inputs, without server round trips.

image::img/drawings/browser-9.jpg[]

It all started with simple DOM manipulations.
...
We did AJAX calls
to submit forms
to fetch some server-side templated HTML,
