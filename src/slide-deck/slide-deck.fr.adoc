...
😎 Il y a beaucoup trop de hype et d'effets de mode, on est devenus complètement addicts à nos outils.
...
Bon par contre, ça nous pousse à faire passer l'expérience de développement avant celle des utilisateurs du produit qu'on développe.
...
On va préfixer nos recherches sur StackOverflow avec angular, react ou vue
...
Slots ?
React +1 : démocratisation du "composant partout"
...
La manière dont vous codez vos templates (et vos composant de manière plus générale) n'a que très peu d'impacts sur l'utilisateur

image::img/drawings/bookshelf-2.jpg[]

=> Angular & Vue : ViewModel avec détection de changement
=> Avec Vue qui propose un modèle de SFC
Première prez de React 2013
On modifie souvent le ViewModel et le template en même temps
=> JSX (j'aime et j'aime pas)
=> "Fork de JavaScript"
Pas de soucis avec la separation of concerns
Mais bcp d'impact sur l'outillage de l'écosystème (compiler, parser, bundlers, linters, editeur de code...)
...
La manière dont vous codez votre CSS n'a que très peu d'impacts sur l'utilisateur
hormis le fait que votre fwk soit capable de manière intelligente de produire un vrai fichier CSS avec le strict minimum
=> impact 1er chargement

image::img/drawings/bookshelf-3.jpg[]

Angular & Vue : système d'encapsulation virtuel
React (juste la vue) => communauté CSS-in-JS lib
=> BEM, encapsulation system
...
Aucun impact sur les devs, c'est sous le capot

image::img/drawings/bookshelf-4.jpg[]

React & Vue => virtual DOM + diffing
=> Modèle super intelligent mais pas silver bullet
Stackovervlow "y'a un vDOM dans Angular ?"
Mais c'est quoi ton problème ?
Le vDOM a été au début un gros argument marketing pour vendre React mais en vrai, on s'en fout.
C'est un détail d'implem, s'ils changent de modèle et que tout marche comme avant mais en plus perfs : bonne nouvelle.
=> microlibs alternatives modernes
Angular a un modèle différent
...
La communauté React est *la plus prolifique en terme de contenus, blogs etc et a pas mal de choses intéressantes* sur la manière d'architecturer ses composants sans forcément que ces patterns soient spécifiques à React.
...
...
Je vais encore vous parler de ma TV.
Parce que j'aime bien ma TV.
Alors, luminosité à 75% sur l'épisode 803 de GoT, #LesVraisSavent
...
Virtual DOM d'un côté
Shadow DOM d'un côté
vDOM: modèle pour modifier le DOM de manière efficace qui forcément du code amené par votre ecosystème
Shadow DOM: Un standard pour isoler le CSS et le DOM de votre composant
Ça n'a rien à voir,
ça se ressemble,
c'est chiant mais c'est la faute de personne au final.
